...
**ממצא-לוואי, לא תוקן (עדיפות נמוכה, דורש הכרעת-דומיין לא קוד):** שיטת ה-`cost` על אותו נכס נתנה ₪905,250 — כמעט חצי משיטת ה-comparables. טבלאות `REPLACEMENT_RATE_PER_SQM`/`LAND_RATE_PER_SQM` ב-`valuationEngine.js` הן ערכים גנריים לכל הארץ, לא מכוילים לפי עיר — לתל אביב זה יפיק תמיד ערך נמוך מדי. לא תוקן כרגע כי זו הכרעת-דומיין (איזה ערכים נכונים בפועל), לא קוד שבור.
