...
// - Writes per-file JSON to public/gisn-extracts/
// - Builds public/gisn-index.json (compact index)
...
function derivePlanKey(filePath, metadata, text) {
  ...
  const mText = String(text || '').match(/(\d{4,8})/)
...
async function ingest(inputDir) {
  ...
  const indexDoc = {
    ...
    items: index,
    disclaimer:
      'Best-effort text extraction from PDFs for appraisal workflow. Treat as unverified; always refer to the original PDFs.'
  }
