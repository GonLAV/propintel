...
export function ARWalkthrough({ property, onClose, sessionId }) {
  ...
  const [arMode, setArMode] = useState('walkthrough')
  const [showOverlays, setShowOverlays] = useState(true)
  const [showGrid, setShowGrid] = useState(false)
  const [measurements, setMeasurements] = useState([])
  const [annotations, setAnnotations] = useState([])
  const [currentMeasurement, setCurrentMeasurement] = useState([])
  const [arSessions, setArSessions] = useKV('ar-sessions', [])
  ...
  const [cursorPositions, _setCursorPositions] = useState(new Map())
...
Return your response as a JSON object with a "suggestions" property containing an array of 5 concise suggestion strings in Hebrew.`
  ...
  const broadcastToParticipants = (type, data) => {
  ...
  const _addAnnotationReply = (annotationId, replyText) => {
    if (!currentUser) return
