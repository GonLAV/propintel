...
export function APIUsageAnalytics() {
  const [usageData, setUsageData] = useKV('api-usage-records', [])
  ...
  const handleExportData = () => {
    const csvContent = [
      ...
      ...filteredData.map(record => [
        ...
        API_CONFIGS[record.apiName]?.name || record.apiName,
        record.endpoint,
                  ...
                  <PieChart>
                    <Pie
                      ...
                      label={({ apiName, requests }) => 
                        `${API_CONFIGS[apiName]?.name}: ${requests}`
                      }
                      outerRadius={80}
                      fill="#8884d8"
                      dataKey="requests"
                    >
                      {analytics.byAPI.map((entry, index) => (
                        <Cell 
                          key={`cell-${index}`} 
                          fill={API_CONFIGS[entry.apiName]?.color || COLORS[index % COLORS.length]} 
                        />
                      ))}
                    </Pie>
                    <Tooltip />
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      labelLine={false}
                      label={({ apiName, cost }) => 
                        `${API_CONFIGS[apiName]?.name}: ₪${cost.toFixed(2)}`
                      }
                      outerRadius={80}
                      fill="#8884d8"
                      dataKey="cost"
        ...
        <TabsContent value="by-api" className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            {analytics.byAPI.map(api => {
              const config = API_CONFIGS[api.apiName]
              return (
            ...
            <CardContent>
              <div className="space-y-2">
                {filteredData.slice(0, 50).map(record => {
                  const config = API_CONFIGS[record.apiName]
                  return (
                    <div 
                      key={record.id}
