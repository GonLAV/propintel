          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {THROTTLE_PRESETS.map((preset) => {
              ...
              return (
                <Card 
                  key={preset.id} 
                  className={`cursor-pointer transition-all hover:shadow-lg hover:scale-105 bg-linear-to-br ${preset.color} border-2 hover:border-primary/50`}
                  onClick={() => {
            ...
            {THROTTLE_PRESETS.map((preset) => {
              ...
              return (
                <TabsContent key={preset.id} value={preset.id} className="space-y-4">
                  <Card className={`bg-linear-to-br ${preset.color} border-2`}>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
